...
// AdvancedMarkerElement requires a mapId. Set VITE_MAPS_MAP_ID in .env and link a
// Cloud Map Style there to restore the dark theme. "DEMO_MAP_ID" works for local dev.
export const MAPS_MAP_ID = import.meta.env.VITE_MAPS_MAP_ID || "DEMO_MAP_ID";
...
export const DEMO_SPOTS = [
  ...
  { lat: 12.9342, lng: 77.621, label: "Koramangala" },
];
...
export function getIssueMeta(issueType) {
  ...
  if (lc.includes("manhole") || lc.includes("sewer")) return ISSUE_META_MAP["Open Manhole"];
  ...
  if (lc.includes("pipe") || lc.includes("leak")) return ISSUE_META_MAP["Water Leakage"];
