  ...
  switch (agent) {
    ...
    case "risk":
      return result.priorityScore != null
        ? `${result.priorityScore}/100 · ${result.publicRiskLevel}`
        : null;
    case "complaint":
    ...
    <div className="pipeline-wrap">
      <div className="pipeline-head">
        <div className="pipeline-title-row">
          {busy ? (
            <>
              <span className="pipeline-spin" aria-hidden>
                ⟳
              </span>{" "}
              AI pipeline running…
            </>
        ...
        {!busy && (
          <button className="pipeline-dismiss" onClick={onDismiss} aria-label="Dismiss">
            ✕
          </button>
        )}
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="pst-circle" aria-label={`${meta.label}: ${step.status}`}>
                <span className={step.status === "running" ? "pst-spin" : ""}>
                  {circleContent(step)}
                </span>
              </div>
              <div className="pst-labels">
                <span className="pst-name">
                  <span className="pst-emoji" aria-hidden>
                    {meta.icon}
                  </span>
                  {meta.label}
                </span>
                {detail && (
                  <span className="pst-detail" title={detail}>
                    {detail}
                  </span>
                )}
              </div>
