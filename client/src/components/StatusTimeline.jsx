...
export default function StatusTimeline({ report }) {
  ...
  const history = statusHistory ?? [
    ...
    ...(report.complaint ? [{ status: "Complaint Drafted", at: report.updatedAt || createdAt, note: `Filed to ${report.department}` }] : []),
  ];
  ...
  if (followUpDate && isFuture(followUpDate)) {
    pendingSteps.push({ status: "Follow-up Due", at: followUpDate, note: "Scheduled check-in with department", future: true });
  }
  if (createdAt && resolvedDays) {
    const expectedAt = new Date(new Date(createdAt).getTime() + resolvedDays * 86_400_000).toISOString();
    if (isFuture(expectedAt)) {
      pendingSteps.push({ status: "Resolved", at: expectedAt, note: `Expected in ${resolvedDays} working days`, future: true });
    }
  }
        ...
        return (
          <div key={i} className={`st-step${step.future ? " st-future" : ""}`}>
            <div className="st-track">
              <div className="st-dot" style={{ borderColor: step.future ? "var(--border)" : meta.color, background: step.future ? "white" : meta.color }}>
            ...
            <div className="st-content">
              <div className="st-row">
                <span className="st-status" style={{ color: step.future ? "var(--muted)" : meta.color }}>
