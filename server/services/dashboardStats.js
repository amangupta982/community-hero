...
export function computeDashboardStats(clusters) {
  ...
  clusters.forEach((c) => { if (c.severity in bySeverity) bySeverity[c.severity]++; });
  ...
  const wardRankings = Object.entries(byWard)
    .map(([ward, items]) => {
      ...
      return {
        ...
        estimatedCostInr:    items.reduce((s, c) => s + (c.riskAssessment?.repairCostEstimate?.high ?? 0), 0),
        slaBreached:         breachedCount,
      };
    })
    .sort((a, b) =>
      (b.criticalCount * 20 + b.avgPriority) - (a.criticalCount * 20 + a.avgPriority)
    )
    .slice(0, 8);

  // Top 8 urgent actions by urgencyScore × priority
  const urgentActions = clusters
    .filter((c) => c.riskAssessment?.urgencyScore != null)
    .sort((a, b) => {
    ...
    .map((c) => ({
      ...
      severity:     c.severity,
      location:     [c.geoContext?.road, c.geoContext?.suburb].filter(Boolean).join(", ") || "Unknown location",
  ...
  return {
    overview: {
      ...
      estimatedCostInr:    clusters.reduce((s, c) => s + (c.riskAssessment?.repairCostEstimate?.high ?? 0), 0),
    },
    trend,
    byType,
    bySeverity,
