...
class ContextAgent extends BaseAgent {
  constructor() { super("context", { maxRetries: 1 }); }

  startMessage() { return "Gathering local context — places, weather, area history…"; }
  ...
  publicResult(r) {
    ...
    return {
      ...
      weather:         r.weather ? { condition: r.weather.condition, temperature: r.weather.temperature } : null,
  ...
  async execute({ lat, lng, issueType }) {
    if (lat == null || lng == null) {
      return { available: false, reason: "No GPS coordinates", places: [], weather: null, historical: { count: 0 } };
    }
