...
export async function runAgentPipeline({ img, lat, lng, reportCount = 1, emit }) {
  ...
  // ── Agent 1: Vision ───────────────────────────────────────────────────────
  const visionResult = await time("vision", () => visionAgent.run({ img }, track));
  ...
  const finalClassification = {
    ...visionResult,
    issueType: verificationResult.confirmsIssueType
      ? visionResult.issueType
      : verificationResult.suggestedIssueType,
    severity: verificationResult.confirmsIssueType
      ? visionResult.severity
      : verificationResult.suggestedSeverity,
  ...
  if (!finalClassification.isCivicIssue) {
    emit({
      ...
      reason: "Verification Agent: not a civic issue",
      verificationResult,
    });
  ...
  // ── Agent 3: Geo ──────────────────────────────────────────────────────────
  const geoResult = await time("geo", () => geoAgent.run({ lat, lng }, track));
